<div class="btn-group blocks">
    ...
    @if ($user->deleted_at)
        ...
        <form id="form_restore_{{$user->id}}" method="POST" action="{{ route('admin.users.restore', $user->id) }}">
            @method('PUT')
            @csrf
        </form>
        ...
        <form id="form_forceDelete_{{$user->id}}" method="POST" action="{{ route('admin.users.forceDelete', $user->id) }}">
            @method('PUT')
            @csrf
        </form>
    @else
        <!-- Edit -->
        <a class='btn btn-primary white_font editQA' href="{{ route('admin.users.edit', $user->id) }}"><i class="fas fa-edit"></i> Edit</a>

        @if ($user->id != 1)
            <button id="deactivate" class="btn btn-danger deactivateQA" delete_id="{{$user->id}}"><i class="fas fa-times"></i> <b>Delete</b></button>

            <form id="form_destroy_{{$user->id}}" method="POST" action="{{ route('admin.users.destroy', $user->id) }}">
                @method('DELETE')
                @csrf
            </form>
        @endif
