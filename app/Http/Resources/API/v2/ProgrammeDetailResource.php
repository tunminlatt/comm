...
class ProgrammeDetailResource extends JsonResource
{
    ...
    public function toArray($request)
    {
        ...
        $banners = $image->get('programmes/'. $id .'/banner');
        if($banners){
            $bannerType = $image->getType($banners[0]['data']);
        }else{
            $bannerType = null;
        }
        ...
        return [
            ...
            'reaction_count' => $this->reaction ? $this->reaction->reaction_count : 0,
            'station_profile_image' =>  $image->getByApi('stations/profile/'. $this->station->id),
            'deep_link' => null,
        ];
    }
}
