...
class AudioRepository
{
    ...
    public function create($payload) {
        return $this->audio->create($payload);
    }
    ...
    public function destroy($id, $type = 1) { // 1 - normal, 2 - force
        if ($type == 1) {
            return $this->audio->destroy($id);
        } else {
            return $this->audio->withTrashed()->find($id)->forceDelete();
        }
    }

    public function restore($id) {
        return $this->audio->withTrashed()->find($id)->restore();
    }

    public function deleteByAudioID ($id) {
        return $this->audio->withTrashed()->find($id)->stationManagers()->detach();
    }

    public function createByAudioID ($id, $payload) {
        return $this->audio->withTrashed()->find($id)->stationManagers()->attach($payload);
    }

    public function share($id) {
        return $this->audio->find($id)->update(['status' => 'public']);;
    }

    public function private($id) {
        return $this->audio->find($id)->update(['status' => 'private']);;
    }
}