...
class ContentRepository
{
    ...
    public function getAllForTable($request, $eagerLoad = [], $withTrash = true, $onlyTrash = false) {
        ...
        $shareStationID = config('app.share_station');
        ...
        return $this->content
                    ...
                    ->when($userTypeID == 2, function ($query) use ($userID, $userStationID, $shareStationID) {
                        return $query->whereIn('station_id', [$userStationID, $shareStationID]);
    ...
    public function create($payload) {
        return $this->content->create($payload);
    }
    ...
    public function destroy($id, $type = 1) { // 1 - normal, 2 - force
        if ($type == 1) {
            return $this->content->destroy($id);
        } else {
            return $this->content->withTrashed()->find($id)->forceDelete();
        }
    }

    public function restore($id) {
        return $this->content->withTrashed()->find($id)->restore();
    }

    public function deleteByContentID ($id) {
        return $this->content->withTrashed()->find($id)->stationManagers()->detach();
    }

    public function createByContentID ($id, $payload) {
        return $this->content->withTrashed()->find($id)->stationManagers()->attach($payload);
    }

    public function share($id) {
        return $this->content->find($id)->update(['status' => 'public']);;
    }

    public function private($id) {
        return $this->content->find($id)->update(['status' => 'private']);;
    }
}
